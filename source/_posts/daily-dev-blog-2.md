...
#### # 10시에 받을 메일을 11시 넘어서 받는다　→　Divide and Conquer
...
#### # 제목없는 글? 블로그 RSS파싱 오류?　→　언제나 신경써야 하는 예외처리(try-catch)
...
#### # 메일 내에는 class 적용 불가　→　Inline Style 처리
...
#### # 메일을 보냈으나 스팸으로 처리된다　→　도메인 구입과 DKIM 설정
...
#### # Elastic Stack을 사용할수 없다.　→　직접 만들어버리자!
...
#### # 구독해제가 아닌 자체 수신거부는 어찌 처리할까?　→　AWS 반송/거부 확인 메일
